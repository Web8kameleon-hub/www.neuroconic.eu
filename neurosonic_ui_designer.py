...
class UIDesignEngine:
    ...
    # Lloje widget-esh të lejuara nga plani i LLM-it. Çdo lloj i panjohur
    # bie mbrapa te "markdown" në menyre te sigurt (nuk thyhet asgjë).
    ...
    def generate_schema(
        ...
        widget_plan: list[dict[str, Any]] | None = None,
        ...
        if widget_plan:
            widgets = self._build_widgets_from_plan(widget_plan, prompt)
        else:
        ...
        return {
            "schema_version": "1.0",
            "owner_id": owner_id,
            "title": title_override or preferences.get("title", "My Neurosonic Panel"),
    ...
    def _build_widgets_from_plan(
        self, widget_plan: list[dict[str, Any]], prompt: str
    ) -> list[dict[str, Any]]:
        """Normalizon një plan widget-esh të propozuar nga LLM.

        Nuk i beson verbërisht të dhënave të LLM: çdo widget kalon nëpër
        validim të fushave (id, type, title, span) dhe llojet e panjohura
        binden ne 'markdown' në menyrë të sigurt, kurrë nuk thyhet skema.
        """
        widgets: list[dict[str, Any]] = []
        for index, item in enumerate(widget_plan):
            if not isinstance(item, dict):
                continue

            raw_type = str(item.get("type", "markdown")).strip().lower()
            widget_type = raw_type if raw_type in self.ALLOWED_WIDGET_TYPES else "markdown"
            widget_id = str(item.get("id") or f"widget_{index + 1}")
            title = str(item.get("title") or widget_type.replace("-", " ").title())
            ...
            widget: dict[str, Any] = {
                ...
                "title": title,
                "span": {
                    "col": max(1, min(12, int(item.get("col", 12) or 12))),
                    "row": max(1, int(item.get("row", index + 1) or index + 1)),
                },
            }

            if item.get("source"):
                widget["source"] = str(item["source"])
            if item.get("action"):
                widget["action"] = str(item["action"])
            if widget_type == "markdown":
                widget["content"] = str(item.get("content") or prompt[:500])
